package com.carma_cam.carmacam.utils;
...
public class CircularEncoder {
    ...
    public CircularEncoder(int width, int height, int bitRate, int frameRate, int desiredSpanSec,
                           Callback cb, LocationServiceCall locationObject, int drivemode, int hazard) throws IOException {
        ...
        // we've initialized.
        mEncoderThread = new EncoderThread(mEncoder, encBuffer, desiredSpanSec, cb, locationObject, frameRate, drivemode, hazard);
    ...
    private static class EncoderThread extends Thread {
        ...
        private int mImmediateHazard;

        private LocationServiceCall mlocationServiceCallObject;


        public EncoderThread(MediaCodec mediaCodec, CircularEncoderBuffer encBuffer, int timeSpan,
                             CircularEncoder.Callback callback, LocationServiceCall locationObject, int frameRate, int drivemode, int hazard) {
            mEncoder = mediaCodec;
            ...
            mImmediateHazard = hazard;

            mlocationServiceCallObject = locationObject;
            ...
            while (true) {
                ...
                if (encoderStatus == MediaCodec.INFO_TRY_AGAIN_LATER) {
                    ...
                    if (mBufferInfo.size != 0) {
                        ...
                        double nSpeed = mlocationServiceCallObject.getmSpeed();
                        mEncBuffer.addSpeed(nSpeed);
        ...
        void saveVideo(File outputFile) {
            ...
            long signal = mEncBuffer.getStartTimeStampUsec() + mEncBuffer.computeTimeSpanUsec() - mTimeSpan * 1000000;

            int index = mEncBuffer.getFirstIndex();
            mSpeed = new double[mFrameRate * mTimeSpan * 2];
            int endSpeedIndex = 0;
            ...
            try {
                ...
                do {
                    ...
                    if(info.presentationTimeUs > signal){
                        muxer.writeSampleData(videoTrack, buf, info);
                        mSpeed[endSpeedIndex] = mEncBuffer.getSpeed(index); // speed tracking from buffer meta data
                        endSpeedIndex++;

                    }
                    Log.d("DEBUG", info.presentationTimeUs + " " + signal);
...
//                if (mImmediateHazard == 0 && (mDriveMode == 0 || mDriveMode == 3)) {
//                    SpeedWatermark watermark = new SpeedWatermark(outputFile, mSpeed, endSpeedIndex, mFrameRate);
//                    watermark.start();
//                }
            }
            if (VERBOSE) {
